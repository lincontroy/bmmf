...
class MerchantConfirmedDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0)" class="btn btn-red d-flex align-items-center justify-content-center py-2 rounded-3 w-px-40 m-1 changeStatus" action-type="suspend" title="' . localize("Suspend") . '" data-action="' . route('admin.merchant.account.update', ['merchant' => $query->id, 'set_status' => MerchantApplicationStatusEnum::SUSPEND->value]) . '"><i class="fa fa-ban" aria-hidden="true"></i></a>';
                $button .= '</div>';
                return $button;
            })
            ...
            ->editColumn('status', function ($query) {
                return '<span class="badge bg-label-success py-2 w-px-75">' .
                MerchantApplicationStatusEnum::APPROVED->name .
                    '</span>';
            })
            ->setRowId('id')
            ->addIndexColumn()
    ...
    {
        $attributes['status'] = MerchantApplicationStatusEnum::APPROVED->value;
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('store_name')->title(localize('Name')),
            ...
            Column::computed('logo')->title(localize('Logo')),
