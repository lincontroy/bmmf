...
class TransferDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0)" class="btn btn-navy d-flex align-items-center justify-content-center py-2 rounded-3 w-px-40 m-1 viewDetails" title="'.localize("Details").'" data-action="' . route('transfer.show', ['transfer' => $query->id]) . '"><i class="fa fa-eye"></i></a>';
                $button .= '</div>';
            ...
            ->editColumn('sender_user_id', function ($query) {
                return $query->senderInformation['first_name'] . " " . $query->senderInformation['last_name'];
            })
            ->editColumn('receiver_user_id', function ($query) {
                return $query->receiverInformation['first_name'] . " " . $query->receiverInformation['last_name'];
            })
    ...
    {
        return $model->newQuery()->with(['senderInformation', 'receiverInformation']);
    }
