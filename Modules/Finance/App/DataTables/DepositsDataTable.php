    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                if ($query->status->value === DepositEnum::PENDING->value) {
                    $button .= '<a href="javascript:void(0)" class="btn btn-navy d-flex align-items-center justify-content-center py-2 rounded-3 w-px-40 m-1 deposit-confirm" action-type="confirm" title="' . localize("Confirm") . '" data-action="' . route('deposit.update', ['deposit' => $query->id, 'set_status' => DepositEnum::CONFIRM]) . '"><i class="fa fa-check"></i></a>';
                    $button .= '<a href="javascript:void(0)" class="btn btn-red d-flex align-items-center justify-content-center py-2 rounded-3 w-px-40 m-1 deposit-confirm" action-type="cancel" title="' . localize("Cancel") . '" data-action="' . route('deposit.update', ['deposit' => $query->id, 'set_status' => DepositEnum::CANCEL]) . '"><i class="fa fa-ban" aria-hidden="true"></i></a>';
                }
            ...
            ->editColumn('status', function ($query) {
                ...
                if ($query->status->value === DepositEnum::CONFIRM->value) {
                    $button = '<span class="badge bg-label-success py-2 w-px-75">' . DepositEnum::CONFIRM->name . '</span>';
                } else

                if ($query->status->value === DepositEnum::CANCEL->value) {
                    $button = '<span class="badge bg-label-danger py-2 w-px-75">' . DepositEnum::CANCEL->name . '</span>';
                } else {
                    $button = '<span class="badge bg-label-warning py-2"><i class="fa fa-spinner fa-spin"></i> ' . DepositEnum::PENDING->name . '</span>';
                }
