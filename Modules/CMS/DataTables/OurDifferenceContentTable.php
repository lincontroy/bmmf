...
class OurDifferenceContentTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-our-difference-content-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.our-difference.content.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.our-difference.content.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="' . localize("Delete") . '" data-action="' . route('admin.cms.our-difference.content.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';

                $button .= '</div>';

                return $button;
            })
            ->addColumn('our_difference_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'button_link', 'our_difference_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->ourDifferenceContent()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('our-difference-content-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('our_difference_image')->title(localize('Image')),
