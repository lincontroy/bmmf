...
class OurServiceTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-our-service-button" title="'.localize("Edit").'" data-action="' . route('admin.cms.our-service.content.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.our-service.content.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="'.localize("Delete").'" data-action="' . route('admin.cms.our-service.content.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';

                $button .= '</div>';

                return $button;
            })
            ->addColumn('our_service_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'our_service_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->ourService()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('our-service-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('our_service_image')->title(localize('Image')),
