...
class WhyChoseContentTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-why-chose-content-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.why-chose.content.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.why-chose.content.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="' . localize("Delete") . '" data-action="' . route('admin.cms.why-chose.content.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';
            ...
            ->addColumn('why_chose_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'button_link', 'why_chose_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->whyChoseContent()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('why-chose-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('article_name')->title(localize('Why Chose')),
            Column::make('why_chose_image')->title(localize('Image')),
