...
class CustomerSatisfyContentTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-satisfied-customer-content-button" title="'.localize("Edit").'" data-action="' . route('admin.cms.satisfied-customer.content.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.satisfied-customer.content.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="'.localize("Delete").'" data-action="' . route('admin.cms.satisfied-customer.content.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';
            ...
            ->addColumn('customer_name', function ($query) {
                if ($query->articleData) {
                    return $query->articleData->keyBy('slug')['name']->content ?? null;
                }
                return null;
            })
            ->addColumn('company_name', function ($query) {
                if ($query->articleData) {
                    return $query->articleData->keyBy('slug')['company_name']->content ?? null;
                }
                return null;
            })
            ->addColumn('company_url', function ($query) {
            ...
            ->addColumn('company_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'company_url', 'company_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->customerSatisfyContent()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('satisfied-customer-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('article_name')->title(localize('Customer Content')),
            Column::make('customer_name')->title(localize('Customer')),
            Column::make('company_name')->title(localize('Company')),
            Column::make('company_url')->title(localize('Url')),
            Column::make('company_image')->title(localize('Image')),
    ...
    {
        return 'CUSTOMER_SATISFY_' . date('YmdHis');
    }
