...
class PackageBannerTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                if ($query->slug == 'package_banner') {
                    $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-package-banner-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.package.banner.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.package.banner.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';
                } elseif ($query->slug == 'package_header') {
                    $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-package-header-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.package.header.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.package.header.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';
                }
            ...
            ->addColumn('package_banner_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'package_banner_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->packageBanner()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('package-banner-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('package_banner_image')->title(localize('Image')),
    ...
    {
        return 'PackageBanner_' . date('YmdHis');
    }
