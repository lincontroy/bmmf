...
class HomeSliderTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-home-slider-button" title="'.localize("Edit").'" data-action="' . route('admin.cms.home-slider.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.home-slider.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="'.localize("Delete").'" data-action="' . route('admin.cms.home-slider.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';
            ...
            ->addColumn('slider_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'button_link', 'slider_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->homeSlider()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('home-slider-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('article_name')->title(localize('Slider Name')),
            Column::make('slider_image')->title(localize('Slider Image')),
