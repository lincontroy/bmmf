...
class InvestmentTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-investment-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.investment.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.investment.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';
            ...
            ->addColumn('investment_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'button_link', 'investment_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->investment()->newQuery();
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('investment_image')->title(localize('Investment Image')),
