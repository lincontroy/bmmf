...
class OurRateTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-our-rate-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.our-rate.content.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.our-rate.content.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="' . localize("Delete") . '" data-action="' . route('admin.cms.our-rate.content.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';
    ...
    {
        return $model->ourRate()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('our-rate-table')
