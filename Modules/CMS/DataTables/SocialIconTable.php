...
class SocialIconTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-social-icon-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.social-icon.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.social-icon.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="' . localize("Delete") . '" data-action="' . route('admin.cms.social-icon.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';
            ...
            ->addColumn('social_icon_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'button_link', 'social_icon_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->socialIcon()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('social-icon-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('article_name')->title(localize('Social Icon')),
            Column::make('social_icon_image')->title(localize('Social Icon Image')),
            Column::make('button_link')->title(localize('Button Link')),
    ...
    {
        return 'HomeSliders_' . date('YmdHis');
    }
