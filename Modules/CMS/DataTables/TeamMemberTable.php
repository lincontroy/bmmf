...
class TeamMemberTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-team-member-content-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.team-member.content.update', ['team_member' => $query->id]) . '" data-route="' . route('admin.cms.team-member.content.edit', ['team_member' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="' . localize("Delete") . '" data-action="' . route('admin.cms.team-member.content.destroy', ['team_member' => $query->id]) . '"><i class="fa fa-trash"></i></a>';
            ...
            ->editColumn('avatar', function ($query) {
                ...
                if ($query->avatar) {
                    return '<img src="' . storage_asset($query->avatar) . '" class="dataTable-image-preview" title="' . $query->name . '">';
                }
                return null;
            })
    ...
    public function query(TeamMemberRepositoryInterface $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('team-member-content-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('designation')->title(localize('Designation')),
            Column::make('avatar')->title(localize('Avatar')),
    ...
    {
        return 'OurDifference_' . date('YmdHis');
    }
