...
class MerchantContentTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0);" class="btn btn-info-soft btn-sm m-1 edit-merchant-content-button" title="' . localize("Edit") . '" data-action="' . route('admin.cms.merchant.content.update', ['article' => $query->id]) . '" data-route="' . route('admin.cms.merchant.content.edit', ['article' => $query->id]) . '"><i class="fa fa-edit"></i></a>';

                $button .= '<a href="javascript:void(0);" class="btn btn-danger-soft btn-sm m-1 delete-button" title="' . localize("Delete") . '" data-action="' . route('admin.cms.merchant.content.destroy', ['article' => $query->id]) . '"><i class="fa fa-trash"></i></a>';

                $button .= '</div>';

                return $button;
            })
            ->addColumn('merchant_image', function ($query) {
            ...
            ->setRowId('id')
            ->addIndexColumn()
            ->rawColumns(['action', 'status', 'merchant_image']);

    }

    public function query(ArticleRepositoryInterface $model): QueryBuilder
    {
        return $model->merchantContent()->newQuery();
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('merchant-content-table')
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('article_name')->title(localize('Merchant Name')),
            Column::make('merchant_image')->title(localize('Merchant Image')),
