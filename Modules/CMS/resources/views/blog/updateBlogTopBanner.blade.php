...
@endphp
<div class="modal fade" id="updateBlogTopBanner" tabindex="-1" aria-labelledby="updateBlogTopBannerLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content radius-35">
            <div class="modal-header p-4">
                <h5 class="modal-title text-color-5 fs-20 fw-medium" id="modelTopBannerLabel">
                    {{ localize('Update Blog Top Banner') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4 pt-2">
                <form action="#" method="post" class="needs-validation" data="showTopBannerCallBackData"
                    id="blog-top-banner-form" novalidate="" data-insert="#" enctype="multipart/form-data"
                    data-getData="{{ route('admin.cms.blog.top_banner.getArticleLang', [':language', ':article']) }}">
                    @csrf
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="blog_top_banner">{{ localize('Top Banner') }} <span
                                        class="text-danger">*</span>
                                </label>
                                <input type="text" name="blog_top_banner" id="blog_top_banner"
                                    value="{{ old('name') }}"
                                    class="custom-form-control form-control @error('blog_top_banner') is-invalid @enderror"
                                    placeholder="{{ localize('enter top banner') }}" required />
                                <div class="invalid-feedback" role="alert">
                                    @error('blog_top_banner')
                                        {{ $message }}
                                    @enderror
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="blog_top_banner_image">{{ localize('Image') }}
                                </label>
                                <div class="" id="preview_file_blog_top_banner_image">
                                </div>
                                <div class="custom-file-button position-relative mb-3">
                                    <input type="file" name="blog_top_banner_image" id="blog_top_banner_image"
                                        accept="image/*"
                                        class="custom-form-control file-preview @error('blog_top_banner_image') is-invalid @enderror"
                                        data-previewDiv="preview_file_blog_top_banner_image" />
                                    <input type="hidden" name="old_image" value="" />
                                    <div class="invalid-feedback" role="alert">
                                        @error('blog_top_banner_image')
                                            {{ $message }}
                                        @enderror
                                    </div>
                                </div>
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="blog_top_banner_title">{{ localize('Top Banner Title') }} <span
                                        class="text-danger">*</span>
                                </label>
                                <input type="text" name="blog_top_banner_title" id="blog_top_banner_title"
                                    value="{{ old('blog_top_banner_title') }}"
                                    class="custom-form-control form-control @error('blog_top_banner_title') is-invalid @enderror"
                                    placeholder="{{ localize('enter top banner title') }}" required />
                                <div class="invalid-feedback" role="alert">
                                    @error('blog_top_banner_title')
                                        {{ $message }}
                                    @enderror
                                </div>
                            </div>
