...
@endphp
<div class="modal fade" id="updateFaqHeader" tabindex="-1" aria-labelledby="updateFaqHeaderLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content radius-35">
            <div class="modal-header p-4">
                <h5 class="modal-title text-color-5 fs-20 fw-medium" id="modelHeaderLabel">
                    {{ localize('Update Faq Header') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4 pt-2">
                <form action="#" method="post" class="needs-validation" data="showHeaderCallBackData"
                    id="faq-header-form" novalidate="" data-insert="#" enctype="multipart/form-data"
                    data-getData="{{ route('admin.cms.faq.header.getArticleLang', [':language', ':article']) }}">
                    @csrf
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="faq_header">{{ localize('Header') }} <span class="text-danger">*</span>
                                </label>
                                <input type="text" name="faq_header" id="faq_header" value="{{ old('name') }}"
                                    class="custom-form-control form-control @error('faq_header') is-invalid @enderror"
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="faq_header_title">{{ localize('Header title') }} <span
                                        class="text-danger">*</span>
                                </label>
                                <input type="text" name="faq_header_title" id="faq_header_title"
                                    value="{{ old('name') }}"
                                    class="custom-form-control form-control @error('faq_header_title') is-invalid @enderror"
                                    placeholder="{{ localize('enter header title') }}" required />
                                <div class="invalid-feedback" role="alert">
                                    @error('faq_header_title')
                                        {{ $message }}
                                    @enderror
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="faq_header_content">{{ localize('Header Content') }} <span
                                        class="text-danger">*</span>
                                </label>
                                <input type="text" name="faq_header_content" id="faq_header_content"
                                    value="{{ old('name') }}"
                                    class="custom-form-control form-control @error('faq_header_content') is-invalid @enderror"
                                    placeholder="{{ localize('enter header content') }}" required />
                                <div class="invalid-feedback" role="alert">
                                    @error('faq_header_content')
                                        {{ $message }}
                                    @enderror
                                </div>
                            </div>
                    ...
                    <div class="d-flex flex-row gap-3">
                        ...
                        <button type="submit" class="actionBtn btn btn-save py-2 w-75"
                            id="faqHeaderFormActionBtn">{{ localize('Create') }}</button>
                    </div>
                </form>
            </div>
        </div>
