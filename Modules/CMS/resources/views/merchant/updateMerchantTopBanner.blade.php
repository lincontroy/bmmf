...
<div class="modal fade" id="updateMerchantTopBanner" tabindex="-1" aria-labelledby="updateMerchantTopBannerLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content radius-35">
            <div class="modal-header p-4">
                <h5 class="modal-title text-color-5 fs-20 fw-medium" id="modelTopBannerLabel">
                    {{ localize('Update Merchant Top Banner') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4 pt-2">
                <form action="#" method="post" class="needs-validation" data="showTopBannerCallBackData"
                    id="merchant-top-banner-form" novalidate="" data-insert="#" enctype="multipart/form-data"
                    data-getData="{{ route('admin.cms.merchant.top-banner.getArticleLang', [':language', ':article']) }}">
                    @csrf
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="merchant_top_banner">{{ localize('Top Banner') }} <span class="text-danger">*</span>
                                </label>
                                <input type="text" name="merchant_top_banner" id="merchant_top_banner"
                                    value="{{ old('name') }}"
                                    class="custom-form-control form-control @error('merchant_top_banner') is-invalid @enderror"
                                    placeholder="{{ localize('enter top banner') }}" required />
                                <div class="invalid-feedback" role="alert">
                                    @error('merchant_top_banner')
                                        {{ $message }}
                                    @enderror
                                </div>
                            </div>
                        ...
                        <div class="col-12 col-md-6">
                            <div class="mb-3">
                                <label class="col-form-label text-start text-color-1 fs-16 fw-medium"
                                    for="merchant_top_banner_title">{{ localize('Title') }} <span
                                        class="text-danger">*</span>
                                </label>
                                <input type="text" name="merchant_top_banner_title" id="merchant_top_banner_title"
                                    value="{{ old('name') }}"
                                    class="custom-form-control form-control @error('merchant_top_banner_title') is-invalid @enderror"
                                    placeholder="{{ localize('enter title') }}" required />
                                <div class="invalid-feedback" role="alert">
                                    @error('merchant_top_banner_title')
                                        {{ $message }}
                                    @enderror
                                </div>
                            </div>
                    ...
                    <div class="d-flex flex-row gap-3">
                        ...
                        <button type="submit" class="actionBtn btn btn-save py-2 w-75"
                            id="merchantTopBannerFormActionBtn">{{ localize('Create') }}</button>
                    </div>
                </form>
            </div>
        </div>
