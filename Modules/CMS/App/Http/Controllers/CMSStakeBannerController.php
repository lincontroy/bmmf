...
use Modules\CMS\App\Http\Requests\CMSStakeBannerRequest;
use Modules\CMS\App\Services\CMSStakeBannerService;

class CMSStakeBannerController extends Controller
{
    use ResponseTrait;
    ...
    /**
     * CMSStakeBannerController constructor
     *
     */
    public function __construct(
        private CMSStakeBannerService $cmsStakeBannerService,
        ...
        $this->mapActionPermission = [
            'edit'           => PermissionMenuEnum::CMS_STAKE->value . '.' . PermissionActionEnum::READ->value,
            'update'         => PermissionMenuEnum::CMS_STAKE->value . '.' . PermissionActionEnum::READ->value,
            'getArticleLang' => PermissionMenuEnum::CMS_STAKE->value . '.' . PermissionActionEnum::READ->value,
        ];
    }
    ...
    public function edit(int $articleId): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => localize("Stake Banner Data"),
            'title'   => localize("Stake Banner"),
            'data'    => $data,
        ]);
    }
     ...
     *
     * @param  CMSStakeBannerRequest  $request
     * @param  int  $languageId
     * @return RedirectResponse
     * @throws Exception
     */
    public function update(CMSStakeBannerRequest $request, int $articleId): JsonResponse
    {
        $data               = $request->validated();
        $data['article_id'] = $articleId;

        $article = $this->cmsStakeBannerService->update($data);

        return response()->json([
            'success' => true,
            'message' => localize("Stake Banner update successfully"),
            'title'   => localize("Stake Banner"),
            'data'    => $article,
        ]);
    }
