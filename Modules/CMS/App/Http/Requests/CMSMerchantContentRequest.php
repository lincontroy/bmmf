...
class CMSMerchantContentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $uniqueRule = Rule::unique(Article::class, 'article_name')
            ->where('slug', ArticleTypeEnum::MERCHANT_CONTENT->value);
        ...
        return [
            "merchant_content"        => ["required", "string", $uniqueRule],
            'image'                   => [$imageRule, 'file', 'mimes:jpeg,jpg,png,gif', 'max:5120'],
            'language_id'             => ['required', 'integer', Rule::exists(Language::class, 'id')],
            "merchant_content_header" => ["required", "string"],
            "merchant_content_body"   => ["required", "string"],
