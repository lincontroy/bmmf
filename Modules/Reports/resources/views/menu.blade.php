...
@endphp
@if (
    $_auth_user->can(PermissionMenuEnum::REPORTS_TRANSACTION->value . '.' . PermissionActionEnum::READ->value) ||
        $_auth_user->can(PermissionMenuEnum::REPORTS_INVESTMENT->value . '.' . PermissionActionEnum::READ->value) ||
        $_auth_user->can(PermissionMenuEnum::REPORTS_FEES->value . '.' . PermissionActionEnum::READ->value) ||
        $_auth_user->can(PermissionMenuEnum::REPORTS_LOGIN_HISTORY->value . '.' . PermissionActionEnum::READ->value))
    <!-- Reports Exchange -->
    <li>
        <a class="has-arrow material-ripple" href="#">
            ...
            <span class="lh-1 ps-2">{{ localize('Reports') }}</span>
        </a>
        <ul class="nav-second-level mm-collapse">
            @if ($_auth_user->can(PermissionMenuEnum::REPORTS_TRANSACTION->value . '.' . PermissionActionEnum::READ->value))
                <li>
                    <a href="{{ route('transactions-logs') }}">{{ localize('transactions') }}</a>
                </li>
            @endif
            @if ($_auth_user->can(PermissionMenuEnum::REPORTS_INVESTMENT->value . '.' . PermissionActionEnum::READ->value))
                <li>
                    <a href="{{ route('investment-history') }}">{{ localize('investments') }}</a>
                </li>
            @endif
            @if ($_auth_user->can(PermissionMenuEnum::REPORTS_FEES->value . '.' . PermissionActionEnum::READ->value))
                <li>
                    <a href="{{ route('fees-history') }}">{{ localize('fees') }}</a>
                </li>
            @endif
            @if ($_auth_user->can(PermissionMenuEnum::REPORTS_LOGIN_HISTORY->value . '.' . PermissionActionEnum::READ->value))
                <li>
                    <a href="{{ route('login-history') }}">{{ localize('Login History') }}</a>
                </li>
            @endif
        </ul>
    </li>
