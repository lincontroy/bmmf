...
class B2xClosedLoanDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $button .= '<a href="javascript:void(0)" class="btn btn-navy d-flex align-items-center justify-content-center py-2 rounded-3 m-1 userInfo" title="' . localize("Information") . '" data-id="' . $query->customer_id . '" data-action="' . route('b2x-loan.getUser') . '"><i class="fa fa-info-circle"></i></a>';
                $button .= '</div>';
                return $button;
            })
            ...
            ->editColumn('status', function ($query) {
                ...
                return '<span class="badge bg-label-danger py-2">' . B2xLoanStatusEnum::CLOSED->name . '</span>';
    ...
    {
        return $model->newQuery()->where('status', B2xLoanStatusEnum::CLOSED->value)->with(['packageInfo',
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('paid_installment')->title(localize('Paid Instal.')),
            Column::make('remaining_installment')->title(localize('Remain. Instal.')),
            Column::make('status')->addClass('text-center')->title(localize('Status')),
