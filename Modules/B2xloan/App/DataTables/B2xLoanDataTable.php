...
class B2xLoanDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                if ($query->status->value === B2xLoanStatusEnum::PENDING->value) {
                    $button .= '<a href="javascript:void(0)" class="btn btn-navy d-flex align-items-center justify-content-center py-2 rounded-3 w-px-40 m-1 changeStatus" action-type="confirm" title="'.localize("Approve").'" data-action="' . route(
                            'b2x-loans.update',
                            ['b2x_loans' => $query->id, 'set_status' => B2xLoanStatusEnum::APPROVED->value]
                        ) . '"><i class="fa fa-check"></i></a>';

                    $button .= '<a href="javascript:void(0)" class="btn btn-purple  text-white d-flex align-items-center justify-content-center py-2 rounded-3 w-px-40 m-1 changeStatus" action-type="reject" title="'.localize("Reject").'" data-action="' . route(
                            'b2x-loans.update',
                            ['b2x_loans' => $query->id, 'set_status' => B2xLoanStatusEnum::REJECTED->value]
                        ) . '"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }

                $button .= '<a href="javascript:void(0)" class="btn btn-navy d-flex align-items-center justify-content-center py-2 rounded-3 m-1 userInfo" title="'.localize("Information").'" data-id="' . $query->customer_id . '" data-action="' . route(
                        'b2x-loan.getUser'
            ...
            ->editColumn('status', function ($query) {
                if ($query->status->value === B2xLoanStatusEnum::APPROVED->value) {
                    ...
                    if ($query->status->value === B2xLoanStatusEnum::PENDING->value) {
                        ...
                        if ($query->status->value === B2xLoanStatusEnum::REJECTED->value) {
    ...
    {
        return $model->newQuery()->where('status', B2xLoanStatusEnum::PENDING->value)->with([
            'packageInfo',
            'customerInfo',
        ])->orderBy('id', 'desc');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('remaining_installment')->addClass('text-center')->title(localize('Remain. Inst.')),
