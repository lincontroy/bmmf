...
class CustomerVerifyDocDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('status', function ($query) {
                return '<a href="#" class="badge bg-label-primary py-2"><span class="spinner-dotted me-2" role="status"> </span><span>' . CustomerVerifyStatusEnum::PROCESSING->name . '</span></a>';
            })
            ->setRowId('id')
            ->addIndexColumn()
    ...
    public function query(Customer $model): QueryBuilder
    {
        return $model->newQuery()
            ->where('verified_status', CustomerVerifyStatusEnum::PROCESSING->value)
            ->with('customerDocs')
            ->orderBy('id', 'desc');
    }
