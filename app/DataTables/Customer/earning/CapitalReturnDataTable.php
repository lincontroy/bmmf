...
class CapitalReturnDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->editColumn('investment_id', function ($query) {
                return '$' . number_format($query->investment->total_invest_amount, 2);
            })
            ...
            ->editColumn('return_amount', function ($query) {
                return '<span class="text-success">$' . number_format($query->return_amount, 2) . '</span>';
            })
            ->editColumn('purchase_quantity', function ($query) {
                return $query->investment->invest_qty;
            })
            ->editColumn('expiry_at', function ($query) {
                return $query->investment->expiry_at;
            })
            ->rawColumns(['user_id', 'interest_amount', 'return_amount'])
            ->addIndexColumn();
    }
     ...
     */
    public function query(CapitalReturn $model): QueryBuilder
    {
        $userId = Auth::user()->user_id;

        return $model->newQuery()->where('user_id', $userId)->where('status', '1')->with(
            ['investment', 'investment.package']
        );
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('investment_id')->title(localize('capital_amount')),
            Column::make('purchase_quantity')->addClass('text-center')->title(localize('qty')),
            Column::make('interest')->addClass('text-center'),
            Column::make('interest_amount')->title(localize('Total Interest Amount(Purchase Qty x Interest Amount )')),
            Column::make('return_amount')->title(localize('return_amount')),
            Column::make('expiry_at'),
        ];
    }
