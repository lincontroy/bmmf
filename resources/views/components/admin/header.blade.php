<nav class="navbar-custom-menu navbar navbar-expand-xl m-0">
    <div class="sidebar-toggle-icon d-block d-lg-none" id="sidebarCollapse">{{ localize('sidebar toggle') }}<span></span></div>
    <!--/.sidebar toggle icon-->
    <!-- Collapse -->
            ...
            <li class="nav-item dropdown user-menu">
                <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown"
                    ...
                    <div class="align-items-center bg-alice-blue d-flex gap-5 admin-bg px-3 py-2 rounded">
                        <div class="align-items-center d-flex gap-2 text-start">
                            <div class="img-user">
                                @if ($_auth_user->image)
                                    <img src="{{ storage_asset($_auth_user->image) }}" class="img-fluid rounded-circle"
                                        alt="" />
                                @else
                            ...
                            <div class="d-flex flex-column gap-1 d-none d-lg-block">
                                <p class="fs-13 fw-bold text-dark lh-sm m-0">
                                    {{ $_auth_user->first_name . ' ' . $_auth_user->last_name }}</p>
                                <span class="fs-11 text-black-50">{{ $_auth_user->email }}</span>
                            </div>
                        </div>
                ...
                <div class="dropdown-menu">
                    ...
                    <div class="user-header">
                        <div class="img-user">
                            @if ($_auth_user->image)
                                <img src="{{ storage_asset($_auth_user->image) }}" alt="" />
                            @else
                        ...
                        <!-- img-user -->
                        <div>
                            <h6>{{ $_auth_user->first_name . ' ' . $_auth_user->last_name }}</h6>
                            <span>{{ $_auth_user->email }}</span>
                        </div>
                    </div>
                    <!-- user-header -->
                    <a href="{{ route('admin.user.show', ['user' => $_auth_user->id]) }}" class="dropdown-item"><svg
                        ...
                        <span>{{ localize('View Profile') }}</span>
                    </a>
                    <a href="{{ route('admin.account-setting', ['id' => $_auth_user->id]) }}" class="dropdown-item">
                    ...
                    <a href="javascript:void(0)" class="dropdown-item" id="themeSwitch" data-light="{{ localize('Light Mode') }}" data-dark="{{ localize('Dark Mode') }}">
                        <i class="far fa-lightbulb fs-15 me-2 w-auto"></i>
                        <span class="ms-0 fs-14 fw-normal">{{ localize('Light Mode') }}</span>
                    </a>
                    <form method="POST" action="{{ route('admin.logout') }}" class="d-inline">
                        @csrf
                        <button class="user-signout">
